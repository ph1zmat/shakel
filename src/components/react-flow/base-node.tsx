  ...
  ({ className, status, ...props }, ref) => {
    return (
      <div
        ref={ref}
        className={cn(
          'bg-card text-card-foreground relative rounded-sm border border-muted-for',
          'hover:bg-accent',
          className,
        )}
        {...props}
      >
        ...
        {status === 'success' && (
          <CircleCheckIcon className="absolute right-0.5 bottom-0.5 size-2 text-green-700 stroke-3" />
        )}
        {status === 'loading' && (
          <Loader2Icon className="absolute -right-0.5 -bottom-0.5 size-2 text-amber-400 stroke-3 animate-spin" />
        )}
      </div>
    );
  },
